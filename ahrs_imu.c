...
float twoKi = 10,twoKp = 0.05;
unsigned short sampleFreq = 20;
...
void Mahony(float gx, float gy, float gz, float ax, float ay, float az, float dt)
{
    ...
    w_xK = gx -w_x0 + Kp*e_x + eInt_x;
    w_yK = gy -w_y0 + Kp*e_y + eInt_y;
    w_zK = gz -w_z0 + Kp*e_z + eInt_z;
    
    cup0 = q0-0.5*(w_xK*q1 + w_yK*q2 + w_zK*q3)*dt;
    cup1 = q1+0.5*(w_xK*q0 + w_zK*q2 - w_yK*q3)*dt;
    cup2 = q2+0.5*(w_yK*q0 - w_zK*q1 + w_xK*q3)*dt;
    cup3 = q3+0.5*(w_zK*q0 + w_yK*q1 - w_xK*q2)*dt;
